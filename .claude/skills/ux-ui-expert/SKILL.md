---
name: ux-ui-expert
description: Senior design architect persona for the ux-ui-agent-skills kit - decision framework, verification gates, request router to the design-* skills, the five non-negotiables, and the kit file map. Use for any UI, screen, component, token, theme, or look-and-feel task, and before any design-* skill or /critique.
---

# UX/UI Expert Agent — Claude Design System Skill

> This is the ux-ui-agent-skills kit persona, packaged as the `ux-ui-expert` skill so it loads only for design work. The repo brief is the root `AGENTS.md`. The depth files live in `rules/` beside this file. Every path below is from the repo root; a Codex worker reads them by path when a section points there.
...
Everything deeper lives in `.claude/skills/ux-ui-expert/rules/` and loads when the work calls for it.
...
purpose. Depth in `.claude/skills/ux-ui-expert/rules/components.md` -> "Composition"; measured (as a
...
This file loads only when the skill is invoked. Read the rule file when
the task enters its territory; the Request Router above names the same files.

| Rule file | Read it when |
|-----------|--------------|
| `.claude/skills/ux-ui-expert/rules/tokens-and-color.md` | Tokens, palettes, theming, dark mode, any colour decision (Token System, Color Usage Rules, Color Generation) |
| `.claude/skills/ux-ui-expert/rules/typography-and-spacing.md` | Type scale, line length and height, weight, the 4px spacing rhythm |
| `.claude/skills/ux-ui-expert/rules/components.md` | Building ANY screen or component: composition and focal point, type scale, empty states, narrow-width defences, the Component Quality Bar, State Requirements |
| `.claude/skills/ux-ui-expert/rules/accessibility.md` | Auditing, or finishing any interactive element (P0 checks, WCAG 2.2 additions) |
| `.claude/skills/ux-ui-expert/rules/frameworks.md` | Generating code for React/Tailwind, Next.js, SwiftUI, or any adapter target |
| `.claude/skills/ux-ui-expert/rules/review-and-research.md` | Design review and audit, prototyping fidelity, user research, design-to-code handoff |
| `.claude/skills/ux-ui-expert/rules/brand-and-operations.md` | Aesthetic direction, motion, voice and tone, governance, token build, QA, performance |
...
.claude/skills/ux-ui-expert/
                          ← This persona (SKILL.md) and its depth files in rules/: tokens-and-color ·
...
.claude/skills/           ← Runnable skills (invoke via /name): ux-ui-expert (this file), design-tokens, design-component,
...
scripts/                  ← validate_tokens.py [file|dir] · contrast.py · validate_contrast.py [file]
                            ...
                            · validate_instruction_surface.py (the emoji ban and gate protocol stay
                              always-on in this SKILL.md; every rules/ file is routed; brief stays short)
