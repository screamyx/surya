...
import { block, fields, split, color, evaluate } from './rust-colors.mjs';
...
/** `TerminalColors`, resolved for one variant. 19 colours the sweep cannot see. */
export function terminalPalette(theme, builtins, seedFields, env) {
  const fieldNames = [...block(theme, 'pub struct TerminalColors').matchAll(/pub (\w+): (?:Hsla|\[Hsla; (\d+)\])/g)];
  const variant = block(builtins, 'fn variant(seed:');
  const source = fields(block(variant, 'terminal: TerminalPalette'));
  // `Theme::from_variant` hardens the terminal foreground only when the variant
  // is not a curated builtin. The two resolved here come straight out of
  // builtins.rs, so they are the builtin and the harden branch never runs.
  if (!block(theme, 'pub(crate) fn from_variant').includes('if !is_curated_builtin')) {
    throw new Error('The terminal foreground hardening is no longer guarded by is_curated_builtin');
  }
  const out = {};
  for (const [, field, arity] of fieldNames) {
    const expression = source[field];
    if (!expression) throw new Error(`variant() no longer seeds terminal.${field}`);
    if (!arity) { out[field] = evaluate(expression, env); continue; }
    // `seed.ansi.map(c)` names a const array of hex strings in builtins.rs.
    const constant = seedFields.ansi;
    const list = builtins.match(new RegExp(`const ${constant}: \\[&str; ${arity}\\] = \\[([^\\]]*)\\];`));
    if (!list) throw new Error(`builtins.rs no longer defines ${constant} as ${arity} colors`);
