...
> Repo convention: rules live in `AGENTS.md`, read natively by Codex and other AGENTS.md tools; `CLAUDE.md` is a one-line `@AGENTS.md` shim for Claude Code. The files under `.claude/rules/` are Claude-only auto-loads; a Codex worker reads them by path when a section above points there.
