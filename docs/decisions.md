...
Amended 2026-09-08 (#226): the card store follows. It is now `<data dir>/cards/<chat id>.jsonl`, where the data dir is `SURYA_DATA_DIR` else `~/.surya` - the same function the mail paths use, so an engine's cards and its mail move together. The sidecar's own default moves with it: the engine names the path explicitly on every real run, so the two agree by construction, but a default only one side moves is the shape that caused the silent mail loss #216 fixed. Files already written at `~/.surya/cards` stay where they are and are not read: a card is a record of what was shown, not state the app needs back, and a one-time move is not worth a migration path that runs on every start forever. There is no separate override for the card store; `SURYA_DATA_DIR` is the knob.
