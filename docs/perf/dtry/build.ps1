# Release build of zeron with the browser feature on dtry, for the frame
...
# Writes: E:\surya-perf-target\release\zeron.exe, E:\surya-perf-build.log.
$ErrorActionPreference = 'Continue'
$log = 'E:\surya-perf-build.log'
$sha = if ($env:ZERON_BUILD_SHA) { $env:ZERON_BUILD_SHA } else { Get-Content E:\surya-perf\SHA -ErrorAction SilentlyContinue }
...
$env:ZERON_BUILD_SHA = $sha
$env:ZERON_BUILD_COMMIT_TIME = [string][int][double]::Parse((Get-Date -UFormat %s))
