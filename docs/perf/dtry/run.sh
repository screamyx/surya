...
envlines=""
for kv in "$@"; do envlines="$envlines$kv\r\n"; done
ssh dtry "New-Item -ItemType Directory -Force -Path E:\\surya-perf-runs | Out-Null; [IO.File]::WriteAllText('E:\\surya-perf-runs\\$name.env', \"$envlines\"); schtasks /delete /tn surya-perf-$name /f 2>\$null | Out-Null; schtasks /create /tn surya-perf-$name /tr \"conhost.exe --headless powershell.exe -NoProfile -ExecutionPolicy Bypass -File E:\\surya-perf\\docs\\perf\\dtry\\run.ps1 -Name $name -Seconds $secs\" /sc once /st 00:00 /it /f | Out-Null; schtasks /run /tn surya-perf-$name | Out-Null; Write-Output started" || exit 1
...
echo "== $name"
ssh dtry "Get-Content E:\\surya-perf-runs\\$name.launch.log; Select-String -Path E:\\surya-perf-runs\\$name.out.log -Pattern 'selftest:|browser: (pump|clock|frame rate|timer|create)|MAIN THREAD QUIET' | ForEach-Object { \$_.Line }; schtasks /delete /tn surya-perf-$name /f | Out-Null"
