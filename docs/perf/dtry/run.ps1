# One measurement run on dtry, session 1: launch zeron with the browser pane
...
param([string]$Name, [int]$Seconds = 40, [string]$Exe = 'E:\surya-perf-target\release\zeron.exe')
...
Remove-Item Env:ZERON_ENGINE -ErrorAction SilentlyContinue
Remove-Item Env:ZERON_ENGINE_TOKEN -ErrorAction SilentlyContinue
...
if ($env:SURYA_PERF_IDLE_SECONDS) {
  $z = Get-Process zeron -ErrorAction SilentlyContinue | Sort-Object WorkingSet64 -Descending | Select-Object -First 1
  if ($z) {
