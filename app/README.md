...
![Surya driving a Claude Code session with a live branch diff sidebar](apps/landing/public/assets/app-screenshot.jpg)
...
```bash
surya status      # local/synced mode and engine status
surya update      # update to the latest release
surya daemon start|stop|restart|status
```
...
`surya login` and `surya logout` refuse to modify credentials while an engine owns the data directory. The desktop app follows the same next-restart profile boundary.

On macOS: use the desktop release, or build `surya` from source and run `surya daemon install` to install the launchd service.
