...
# surya-rpc e2e_driver example proving the doc-queued cross-device command path:
#
#   B queues a Run into the chat doc -> nudge -> A (host) executes via the mock
#   harness -> transcript + session status sync A -> edge -> B.
#
# Both engines run as the SAME user (alice@org1) on different devices — surya's
# one-user-many-devices model; chat/device rooms are claim-on-first-join per user.
#
# Usage: scripts/e2e-smoke.sh
# Env:   SURYA_E2E_EDGE_PORT (default 27640), SURYA_E2E_KEEP_LOGS=1 to keep logs.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
command -v cargo >/dev/null 2>&1 || PATH="$HOME/.cargo/bin:$PATH"
EDGE_PORT="${SURYA_E2E_EDGE_PORT:-27640}"
...
A_DIR=/tmp/e2e-a
B_DIR=/tmp/e2e-b
LOG_DIR="$(mktemp -d /tmp/surya-e2e-logs.XXXXXX)"
...
cleanup() {
  ...
  if [[ "${SURYA_E2E_KEEP_LOGS:-0}" != "1" ]]; then
    rm -rf "$LOG_DIR"
  else
...
echo "build: surya + e2e_driver"
# Two invocations: a single one with `--example` builds ONLY the example
# (the target filter applies across every -p), silently skipping the surya
# bin — the smoke then dies on "No such file or directory".
(cd "$ROOT" && cargo build -q -p surya)
(cd "$ROOT" && cargo build -q -p surya-rpc --example e2e_driver)
ZERON="$ROOT/target/debug/surya"
...
start_engine() { # start_engine <data_dir> <ipc_port> <name> <log>
  SURYA_DATA_DIR="$1" SURYA_IPC_PORT="$2" SURYA_DEVICE_NAME="$3" \
    SURYA_EDGE_URL="$EDGE_URL" SURYA_EDGE_TOKEN="$TOKEN" SURYA_ORG_ID="$ORG" \
    SURYA_HARNESS=mock RUST_LOG=info \
