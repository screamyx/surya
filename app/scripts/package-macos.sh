...
#   target/package/surya-<version>-macos-<arch>.dmg          (user download)
#   target/package/surya-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Surya.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Surya.app"
DMG="$OUT_DIR/surya-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/surya-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/surya" "$APP/Contents/MacOS/surya"
...
# applied ahead of time; dist/surya.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/surya.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/surya.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/surya-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Surya.app
...
dmgbuild.build_dmg(
    ...
    settings={
        ...
        "icon": os.path.join(app, "Contents/Resources/surya.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Surya.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
