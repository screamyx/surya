...
#
# Everything lives under /tmp/surya-demo-*; re-runs reuse it. Ctrl-C cleans up.
set -euo pipefail
cd "$(dirname "$0")/.."

DAEMON_DIR=/tmp/surya-demo-daemon
UI_DIR=/tmp/surya-demo-ui
IPC=27921
DELAY=""
[[ "${1:-}" == "--slow" ]] && DELAY=350

echo "▸ building (first run takes a few minutes)…"
cargo build -p surya -q

echo "▸ starting engine daemon on :$IPC"
env SURYA_DATA_DIR="$DAEMON_DIR" SURYA_IPC_PORT=$IPC SURYA_HARNESS=mock \
  ${DELAY:+SURYA_MOCK_DELAY_MS=$DELAY} RUST_LOG=warn \
  ./target/debug/surya headless &
DAEMON_PID=$!
...
probe() { cargo run -q -p surya-rpc --example rpc_probe -- "ws://127.0.0.1:$IPC" "$@"; }
...
if [[ ! -f "$DAEMON_DIR/.demo-seeded" ]]; then
  ...
  declare -A SPACES=()
  for project in surya soccertcg surya aether; do
  ...
  seed "Native Surya Rust Rewrite"    surya surya/main                 0  run
  seed "Rebalance Player Stats Caps"  soccertcg    surya/rebalance-player-stat-caps  2  run
  seed "Craft Premium TCG Experience" soccertcg    surya/craft-premium-tcg-exp       26 skip
  seed "Initial Context Exploration"  surya        surya/initial-context-exploration 14 skip
  seed "Soccer TCG Repo Creation"     aether       aether/main                       48 skip
  touch "$DAEMON_DIR/.demo-seeded"
fi

echo "▸ opening surya (composer is live — type into it; --slow shows streaming)"
SURYA_DATA_DIR="$UI_DIR" SURYA_IPC_PORT=$IPC RUST_LOG=warn ./target/debug/surya
