...
//! `$SURYA_ADAPTERS_DIR`, spawn it via node, and reach SessionStarted (the
...
//! `cargo test -p surya-harness --test managed_install -- --ignored`.
//!
//! Single-test binary: it mutates SURYA_ADAPTERS_DIR process-wide.

use futures::StreamExt;
use tokio::sync::mpsc;
use tokio_util::sync::CancellationToken;
use surya_harness::{AcpHarness, Harness, RunControls};
use surya_proto::{AgentEvent, RunRequest};
...
async fn managed_install_reaches_session_started() {
    ...
    unsafe {
        std::env::set_var("SURYA_ADAPTERS_DIR", adapters.path());
    ...
    let version_dir = std::fs::read_dir(&root)
        ...
        .path();
    assert!(version_dir.join(".surya-install-ok").exists());
}
