...
//!
//! Single-test binary: it mutates PATH/SHELL/SURYA_* env process-wide.
...
use surya_harness::{AcpHarness, Harness, HarnessError, RunControls};
use surya_proto::{RunRequest, SandboxLevel};
...
async fn silent_npm_enoent_death_surfaces_decoded_error() {
    ...
    unsafe {
        std::env::set_var("SURYA_ADAPTERS_DIR", dir.path().join("adapters"));
        std::env::set_var("SURYA_NO_LOGIN_SHELL", "1");
