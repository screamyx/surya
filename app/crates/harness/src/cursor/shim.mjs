// surya cursor shim — a thin, surya-owned wrapper around the PINNED
...
// every bump). Materialized by surya into the SDK's managed install dir so
...
// Surya runs concurrent shims on the same cwd as a matter of course — the
...
const STATE_BASE =
  process.env.SURYA_CURSOR_STATE_DIR || path.join(os.homedir(), ".surya", "cursor-state");
...
async function start(msg) {
  ...
  const options = {
    ...
    // generateImage has nowhere to land in a surya session (ACP parity).
