...
//! `@cursor/sdk` via a thin surya-owned Node shim (`shim.mjs`, JSONL over
...
//!   (`~/.surya/adapters/…`, [`crate::adapter_install::ensure_installed_shim`])
...
const SHIM_NAME: &str = "surya-cursor-shim.mjs";
...
impl Harness for CursorHarness {
    ...
    /// install surya performs on demand).
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "surya_harness::cursor", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
...
async fn stdin_writer(mut stdin: ChildStdin, mut rx: mpsc::UnboundedReceiver<String>) {
    while let Some(line) = rx.recv().await {
        ...
        if let Err(e) = write.await {
            tracing::debug!(target: "surya_harness::cursor", "stdin write failed (tolerated): {e}");
            return;
        }
    }
            ...
            line = stdout_lines.next_line() => match line {
                Ok(Some(line)) => {
                    ...
                    let Ok(frame) = serde_json::from_str::<Value>(line) else {
                        tracing::debug!(target: "surya_harness::cursor", "unparseable shim frame (skipped)");
                        continue;
                    };
...
fn map_shim_frame(frame: &Value, interrupted: bool) -> Vec<AgentEvent> {
    ...
    match frame.get("ev").and_then(Value::as_str).unwrap_or("") {
        ...
        other => {
            tracing::debug!(target: "surya_harness::cursor", "unknown shim frame (skipped): {other}");
            Vec::new()
        }
    }
}
