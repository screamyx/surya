...
//! [`default_stall_bound`] (`SURYA_OPENCODE_STALL_MS`, 0 disables) errors
...
const STARTUP_TIMEOUT_ENV: &str = "SURYA_OPENCODE_STARTUP_TIMEOUT_SECS";
...
const STALL_ENV: &str = "SURYA_OPENCODE_STALL_MS";
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "surya_harness::opencode", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
...
async fn run_session(session: Session) {
    ...
    if !done_sent {
        ...
        tracing::debug!(target: "surya_harness::opencode", "run loop ended without settling");
    }
    bus_handle.abort();
    server.shutdown(kill_grace).await;
}
