...
/// The scripted question set for the `SURYA_MOCK_QUESTION` variant (exercises
...
impl Harness for MockHarness {
    ...
    async fn run(
        ...
        // `SURYA_MOCK_BROWSER=<url>`: drive the browser pane through the
        ...
        // Optional pacing knob for demos/manual testing: `SURYA_MOCK_DELAY_MS`
        ...
        let delay_ms = std::env::var("SURYA_MOCK_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(0);
        let delay = std::time::Duration::from_millis(delay_ms);

        // Dev/testing knob: `SURYA_MOCK_QUESTION=1` swaps in a run that asks
        ...
        let question_mode = std::env::var("SURYA_MOCK_QUESTION")
            .ok()
        ...
        // Dev/testing knob: `SURYA_MOCK_REPEAT=N` loops the script body N times
        ...
        // at the very end).
        let repeat = std::env::var("SURYA_MOCK_REPEAT")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(1)
            .max(1);
        // Dev/testing knob: `SURYA_MOCK_ERROR=1` appends a scripted error
        // before the terminal Done — the only data-side way to put the
        // transcript ErrorChip on screen with the mock harness.
        let mock_error = std::env::var("SURYA_MOCK_ERROR")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        // Dev/testing knob: `SURYA_MOCK_TABLE=1` appends scripted GFM tables
        ...
        let mock_table = std::env::var("SURYA_MOCK_TABLE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        // Dev/testing knob: `SURYA_MOCK_CARDS=<dir>` appends one A2UI card
        ...
        let card_events: Vec<AgentEvent> = std::env::var("SURYA_MOCK_CARDS")
            .ok()
        ...
        // Dev/testing knob: `SURYA_MOCK_PERMISSION=1` scripts a blocked tool
        ...
        let mock_permission = std::env::var("SURYA_MOCK_PERMISSION")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `SURYA_MOCK_CODE=1` appends rust + ts code blocks
        ...
        let mock_code = std::env::var("SURYA_MOCK_CODE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `SURYA_MOCK_MEND=1` appends a link/list-heavy
        ...
        let mock_mend = std::env::var("SURYA_MOCK_MEND")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `SURYA_MOCK_THINKING=1` appends a markdown-heavy
        ...
        let mock_thinking = std::env::var("SURYA_MOCK_THINKING")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        let thinking_events = mock_thinking
            .then(|| {
                [
                    ...
                    AgentEvent::ToolCall {
                        ...
                        call: surya_proto::ToolCall::Exec {
            ...
            .into_iter()
            .flatten();
        // Dev/testing knob: `SURYA_MOCK_SUBAGENT=1` appends two spawn chips
        ...
        // paced run (`SURYA_MOCK_DELAY_MS`) holds a Running chip long enough
        // to observe.
        let mock_subagent = std::env::var("SURYA_MOCK_SUBAGENT")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
                ...
                vec![
                    ...
                    tag(
                        ...
                        AgentEvent::ToolCall {
                            id: "sub2-burst".into(),
                            call: surya_proto::ToolCall::Exec {
                                command: "cargo test -p surya-doc cadence_burst -- --nocapture".into(),
                            },
                        },
                    ),
                    resolve("mock-sub-2"),
                    ...
                    tag(
                        ...
                        AgentEvent::ToolCall {
                            ...
                            call: surya_proto::ToolCall::Exec {
                                command: "cargo test -p surya-doc cadence_steer -- --nocapture"
                                    .into(),
                            },
                        },
                    ),
        ...
        let events: Vec<Result<AgentEvent, HarnessError>> = body
            ...
            .map(Ok)
            .collect();
        // Dev/testing knob: `SURYA_MOCK_CHARS=N` re-chunks every TextDelta
        // into N-char deltas, so `SURYA_MOCK_DELAY_MS` paces *characters*
        ...
        let chunk_chars = std::env::var("SURYA_MOCK_CHARS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&n| n > 0);
        ...
        // Dev/testing knob: `SURYA_MOCK_SUBAGENT_DELAY_MS` paces TAGGED
        // (subagent) events on their own clock — the parent turn settles at
        // `SURYA_MOCK_DELAY_MS` speed while the background subagents stream
        ...
        let sub_delay_ms = std::env::var("SURYA_MOCK_SUBAGENT_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok());
...
/// The `SURYA_MOCK_CARDS` fixtures as events: a one-line reply, then the
/// card, per file. Unreadable files are skipped with a warning.
fn mock_card_events(dir: &std::path::Path) -> Vec<AgentEvent> {
    let Ok(read) = std::fs::read_dir(dir) else {
        tracing::warn!(dir = %dir.display(), "SURYA_MOCK_CARDS: cannot read dir");
        return Vec::new();
    };
    ...
    for (ix, path) in paths.iter().enumerate() {
        ...
        let Ok(json) = serde_json::from_str::<serde_json::Value>(&text.replace("fixture://", &base))
            ...
            tracing::warn!(path = %path.display(), "SURYA_MOCK_CARDS: not JSON");
            continue;
        };
        let stem = path
            .file_stem()
...
mod tests {
    ...
    /// Same contract as `surya_ui::cards::fixture_base`: odd dir names stay
