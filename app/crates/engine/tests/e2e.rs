...
fn queue_as_viewer(doc: &SessionDoc, id: &str, payload: SessionCommandPayload) {
    ...
    let based_on =
        doc.read_entries()
            .expect("read entries")
            .last()
            .map(|m| surya_doc::CommandBasedOn {
...
async fn steer_with_no_live_run_falls_back_to_new_turn() {
    ...
    // dispatch-as-next-turn, per surya's executor.
    queue_as_viewer(
        handle.doc(),
        "cmd-steer-1",
...
async fn processed_commands_are_skipped_on_redelivery() {
    ...
    let verdict = surya_doc::evaluate_command(
        entry,
        &surya_doc::EvaluationContext {
    ...
    assert_eq!(verdict, surya_doc::CommandDisposition::Skip);
}
...
async fn rpc_surface_over_in_memory_transport() {
    ...
    let harnesses = client
        .call(surya_rpc::methods::LIST_HARNESSES, serde_json::Value::Null)
        .await
        .unwrap();
    assert_eq!(harnesses[0]["id"], "mock");
    let models = client
        .call(
            surya_rpc::methods::LIST_MODELS,
    ...
    let mut sessions_stream = client
        .subscribe(surya_rpc::methods::WATCH_SESSIONS, serde_json::Value::Null)
        .await
        .unwrap();
    let first_sessions = tokio::time::timeout(surya_test_deadlines::WAIT, sessions_stream.recv())
        .await
        .unwrap()
        .unwrap();
    ...
    let mut messages_stream = client
        .subscribe(
            surya_rpc::methods::WATCH_DOC_MESSAGES,
            serde_json::json!({"chatId": CHAT}),
        )
        .await
        .unwrap();
    let initial = tokio::time::timeout(surya_test_deadlines::WAIT, messages_stream.recv())
        .await
        .unwrap()
        .unwrap();
    ...
    let queued = client
        .call(
            surya_rpc::methods::QUEUE_COMMAND,
    ...
    let settled = loop {
        let item = tokio::time::timeout_at(deadline, messages_stream.recv())
            ...
            .expect("stream alive");
        let frame: surya_doc::TranscriptFrame = serde_json::from_value(item).unwrap();
        surya_doc::apply_transcript_frame(&mut materialized, frame).unwrap();
...
async fn respond_input_resolves_pending_question() {
    ...
    impl Harness for AskingHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let answers = (controls.request_input)(vec![surya_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
    ...
    queue_as_viewer(
        ...
        SessionCommandPayload::RespondInput {
            request_id,
            answers: vec![surya_proto::UserInputAnswer {
                question_id: "q1".into(),
                labels: vec!["b".into()],
            }],
        },
        ...
        async fn run(
            ...
            if request.prompt == "second run" {
                ...
                tokio::spawn(async move {
                    ...
                    let _ = (controls.request_input)(vec![surya_proto::UserInputQuestion {
                        id: "q1".into(),
                        header: "Pick".into(),
                        question: "Which one?".into(),
                        options: vec!["a".into(), "b".into()],
...
async fn harness_emitted_input_twin_is_dropped_and_answer_resumes() {
    ...
    impl Harness for DoubleEmitHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let question = surya_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
                    question: "Which one?".into(),
                    options: vec!["a".into(), "b".into()],
...
async fn attachment_upload_then_run_threads_refs_and_paths() {
    ...
    for (seq, data) in [(0, first), (1, second)] {
        client
            .call(
                surya_rpc::methods::UPLOAD_CHUNK,
    ...
    let committed = client
        .call(
            surya_rpc::methods::UPLOAD_COMMIT,
    ...
    // Run with the surya `withAttachments` transport: refs embedded in the
    ...
    let chunk = client
        .call(
            surya_rpc::methods::READ_ATTACHMENT_CHUNK,
...
/// Run with: `cargo test -p surya-engine --test e2e -- --ignored`
#[tokio::test(flavor = "multi_thread")]
