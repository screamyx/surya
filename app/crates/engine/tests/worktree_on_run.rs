...
//! relay RPC), runs there, and stamps the chat row's cwd + `surya/<name>`
...
async fn run_with_worktree_spec_materializes_on_host_and_reuses() {
    ...
    unsafe { std::env::set_var("SURYA_WORKTREES_DIR", &worktrees_root) };
    ...
    // with the actual surya/<name> (the composer only knew the base).
    let chat = core
        .workspace
        .chat(CHAT)
