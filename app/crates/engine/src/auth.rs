...
//! ARCHITECTURE §5). Port of surya's `apps/backend/src/auth.ts`.
//!
...
/// SignedIn{user, orgId?}`). Serializes as the canonical [`surya_proto::AuthState`]
...
impl AuthState {
    ...
    pub fn to_proto(&self) -> surya_proto::AuthState {
        let profile = |user: &AuthUser| surya_proto::UserProfile {
            id: user.id.clone(),
            email: user.email.clone(),
            name: user.name.clone(),
        };
        match self {
            AuthState::SignedOut => surya_proto::AuthState::SignedOut,
            AuthState::NeedsOrganization { user } => surya_proto::AuthState::NeedsOrganization {
                user: profile(user),
            },
            AuthState::SignedIn { user, org_id } => surya_proto::AuthState::SignedIn {
                user: profile(user),
                org_id: org_id.clone(),
            },
        }
...
pub struct AuthConfig {
    ...
    /// Dev-mode bearer/user id (mirrors the old `SURYA_EDGE_TOKEN` behavior).
...
impl Auth {
    ...
    fn finish_sign_in(&self, result: SignInResult, generation: u64) -> Result<(), EngineError> {
        ...
        if sign_in.generation != generation {
            return Err(EngineError::Other(
                "sign-in was canceled — start again from Surya".into(),
            ));
        }
...
/// The relay/room token seam: `Auth` IS a [`surya_rpc::TokenSource`], so the host relay
/// and link cache always dial with a fresh bearer after refreshes.
#[async_trait::async_trait]
impl surya_rpc::TokenSource for Auth {
...
async fn handle_loopback_conn(
    ...
    let (status, body) = if path != "/callback" {
        ...
        let invalid_callback = || {
            (
                "400 Bad Request",
                page("Invalid or expired sign-in link. Start again from Surya."),
            )
        };
        match (code, state) {
            (Some(code), Some(state)) => match auth.take_pending(state) {
                Some(generation) => match auth.exchange_code(code).await {
                    Ok(result) => match auth.finish_sign_in(result, generation) {
                        Ok(()) => (
                            "200 OK",
                            page("Signed in. You can close this tab and return to Surya."),
                        ),
                        Err(err) => {
                            ...
                            (
                                "409 Conflict",
                                page(
                                    "This sign-in was canceled. Start again from Surya if you still want to enable sync.",
                                ),
                            )
                        }
                    },
                    Err(err) => {
                        ...
                        (
                            "502 Bad Gateway",
                            page("Sign-in failed during token exchange — check the Surya logs."),
                        )
                    }
                },
...
mod tests {
    ...
    fn auth_state_serializes_as_proto_shape() {
        ...
        let parsed: surya_proto::AuthState = serde_json::from_value(value).expect("proto parse");
        assert!(matches!(parsed, surya_proto::AuthState::SignedIn { .. }));
        assert_eq!(
