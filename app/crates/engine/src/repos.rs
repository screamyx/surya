...
//! browser (feature-inventory §3.5; port of surya's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.surya/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `surya/<name>` branch. `SURYA_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
    "otter", "harbor", "falcon", "cedar", "meadow", "surya", "delta", "ember", "lynx", "maple",
...
/// worktrees are user-facing working checkouts. `SURYA_WORKTREES_DIR` overrides
/// (test isolation); empty reads as unset.
fn default_worktrees_root() -> PathBuf {
    surya_proto::env_compat::var_os("WORKTREES_DIR")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".surya").join("worktrees"))
}
...
impl Repos {
    ...
    /// comes from `$SURYA_WORKTREES_DIR` or `~/.surya/worktrees`.
    ...
    /// remotes, and tags seed the walk, so Surya's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `surya/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists() && !existing.contains(&format!("surya/{candidate}"))
            {
                name = Some(candidate);
                break;
            }
        ...
        let branch_name = format!("surya/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a surya-created worktree branch after its chat's generated title
    /// (port of surya's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `surya/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("surya/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when surya created it (`surya/…`) — the
    ...
    /// timeout the thread is simply abandoned (the surya backend's disposable
...
/// Turn a generated chat title into the semantic portion of a Surya branch
/// (port of surya's `worktreeBranchFromTitle`). Surya NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
    ...
    format!("surya/{}", if slug.is_empty() { "update" } else { slug })
}
