...
//! (feature-inventory §3.7 "Agent accounts"; port of surya's `agent-accounts.ts`).
//!
...
//!   whole-account session tokens, which surya never reads.
//!
...
//! (`/usage` in Claude Code, `/status` in Codex). Unlike surya (fetch on every
...
/// One saved login (`{slotId}.json`), same field surface as surya's slot files.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct Slot {
    id: String,
...
impl AgentAccounts {
    ...
    /// Cursor: the SDK's own PKCE browser flow, driven through the surya shim
    ...
    async fn start_cursor_login(&self) -> Result<AgentLoginStart, EngineError> {
        ...
        let home = self
            ...
            .join(format!(".login-{login_id}"));
        std::fs::create_dir_all(&home)?;
        let mut cmd = surya_harness::cursor::login_command(&home.join("auth.json"))
            .await
            .map_err(|e| {
                let _ = std::fs::remove_dir_all(&home);
    ...
    /// Lazy TTL sweep (surya uses a background fiber; native reaps on the next
