...
impl AgentStates {
    ...
    pub fn resolve_permission(
        ...
        // The rule is created only for an Allow: "always allow" is the only
        // shape decision 20 gives the card, and a remembered Deny would be a
        // block-list nobody asked for.
        let created_rule = match (remember, decision) {
            (Some(remember), PermissionDecision::Allow) => {
                let rule =
                    AllowRules::from_remember(remember, &pending.request, &pending.cwd);
                Some(
                    self.inner
                        .rules
                        .add(rule)
    ...
    fn build_states(&self, needs_you: &[NeedsYouItem]) -> Vec<AgentStateRow> {
