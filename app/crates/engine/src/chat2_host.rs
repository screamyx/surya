...
//! of [`surya_sync::chat_client::ChatDocSink`] and
//! [`surya_sync::chat_client::CheckpointFetcher`], binding a
...
use surya_doc::SessionDoc;
use surya_sync::chat_client::{ChatDocSink, CheckpointFetcher};
use surya_sync::{DocsStore, SyncError};
...
impl surya_sync::chat_client::ChatTransport for EdgeChatTransport {
...
mod frontier_tests {
    ...
    fn encoded_empty_frontier_is_not_contained() {
        let dir = std::env::temp_dir().join(format!("surya-frontier2-{}", std::process::id()));
    ...
    fn empty_frontier_is_not_contained() {
        let dir = std::env::temp_dir().join(format!("surya-frontier-test-{}", std::process::id()));
        let store = Arc::new(DocsStore::open(&dir).expect("store opens"));
        let doc = Arc::new(SessionDoc::from_doc(loro::LoroDoc::new()));
