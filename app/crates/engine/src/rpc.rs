...
impl RpcService for EngineRpc {
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        ...
        match method {
            ...
            methods::FILES_TREE | methods::FILES_WATCH | methods::FILES_READ
            | methods::FILES_WRITE | methods::FILES_SEARCH | methods::FILES_MUTATE => {
                self.handle_files(method, params).await
            }
            methods::SEARCH_FILES => {
