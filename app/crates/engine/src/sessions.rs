...
//!
//! Pragmatic port of surya's `sessions.ts` (spec: feature-inventory §3.2):
...
//!
//! Scope notes: sessions are keyed by chat id (one live run per chat). Surya's pulse
...
use surya_harness::permission::PermissionGate;
use surya_harness::{CancellationToken, Harness, RunControls, SteerMessage};
use surya_proto::{
...
/// directory — surya sessions.ts:563 "harness session stores are keyed by
...
struct RuntimeConfig {
    ...
    model: Option<String>,
    reasoning: Option<surya_proto::ReasoningLevel>,
    model_options: serde_json::Map<String, serde_json::Value>,
    cwd: String,
    sandbox: surya_proto::SandboxLevel,
    auto_approve: bool,
    worktree: Option<surya_proto::WorktreeSpec>,
}
...
struct Inner {
    ...
    /// (surya kept the same pair on `chats.harness_session_id`). An empty
...
impl SessionsEngine {
    ...
    pub fn any_active(&self) -> bool {
        lock(&self.inner.statuses).values().any(|s| {
            matches!(
                s.status,
                surya_proto::SessionStatus::Working | surya_proto::SessionStatus::AwaitingInput
            )
        })
    }
    ...
    pub async fn interrupt(&self, chat_id: &str) -> Result<bool, EngineError> {
        ...
        // Unpark any blocked question FIRST (mirrors surya: harness teardown can await a
    ...
    /// the remembered harness session (surya: "not just eulogized";
    ...
    pub fn recover_stale(&self) -> Result<usize, EngineError> {
        ...
        for chat_id in stale {
            ...
            // same harness conversation (surya recoverDraft, sessions.ts:538).
...
impl Inner {
    ...
    /// engine restart (surya sessions.ts:1039).
    ...
    /// (surya sessions.ts:736, looked up on every dispatch):
...
impl SubagentSink {
    ...
    fn finish(mut self, device_id: &str, status: MessageStatus) -> Option<String> {
        ...
        let entries = surya_doc::join_continuation_entries(self.doc.read_entries().ok()?);
...
async fn drive_run(
    ...
    // PERSISTENT SESSION (surya runsBySession): a completed turn on a
    ...
    // a session nobody comes back to (surya SESSION_IDLE_MS).
    ...
    // moment output flows again, and nothing is lost. `SURYA_TURN_QUIESCE_MS`
    ...
    let quiesce_after: Option<std::time::Duration> = match std::env::var("SURYA_TURN_QUIESCE_MS")
        .ok()
    ...
    // push the window during real thinking. `SURYA_SELF_TURN_QUIESCE_MS`
    // overrides; 0 falls back to the normal window. An explicit
    // `SURYA_TURN_QUIESCE_MS=0` still disables the watchdog entirely.
    let self_quiesce_after: Option<std::time::Duration> =
        match std::env::var("SURYA_SELF_TURN_QUIESCE_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
        {
            Some(0) => None,
    ...
    let final_status = loop {
        let event: AgentEvent = tokio::select! {
            ...
            // Idle reaper (surya SESSION_IDLE_MS): a parked persistent session
            ...
            _ = tokio::time::sleep_until({
                ...
                && !folded.iter().any(|p| match p {
                    MessagePart::Tool { id, resolved: false, .. } => {
                        id != surya_proto::LIVE_PLAN_TOOL_ID
                    }
        ...
        } = &event
        {
            ...
            if chip_streaming {
                ...
                surya_doc::fold_event_into_parts(&mut folded, &event);
                if !dirty {
                    dirty = true;
            ...
            if let Some(sink) = subagents.get_mut(parent_tool_use_id) {
                ...
                surya_doc::fold_event_into_parts(&mut sink.folded, sub_event);
                sink.dirty = true;
            ...
            let self_continued = idle_since
                ...
                && (matches!(
                    ...
                    AgentEvent::ToolCall { id, .. }
                        if id == surya_proto::LIVE_PLAN_TOOL_ID || !seen_tools.contains(id)
        ...
        // Defensive rule from surya: a mid-run SessionStarted re-emission (Claude SDK
        ...
        if !skip_fold {
            ...
            // journal. To reintroduce: `surya_doc::sidecar_payload(&event)`
...
mod tests {
    ...
    use surya_proto::{HarnessId, RunRequest, SandboxLevel};
    ...
    fn request() -> RunRequest {
        RunRequest {
            ...
            reasoning: Some(surya_proto::ReasoningLevel::High),
    ...
    fn live_routing_requires_the_same_runtime_configuration() {
        ...
        follow_up.reasoning = Some(surya_proto::ReasoningLevel::Medium);
        assert!(!config.can_route(HarnessId::Grok, &follow_up));
