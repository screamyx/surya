...
pub struct ImportReport {
    ...
    /// Deterministic repairs made after mapping so the compiled Surya roles
...
fn convert(theme: NormalizedTheme, options: ImportOptions) -> Result<ImportResult> {
    ...
    let mut output = registry
        .variant(base_id)
        .expect("Surya base theme exists")
        .clone();
    ...
    report.fallbacks.push(
        "surface treatment inferred as opaque because VS Code palettes target solid workbench backgrounds; the user's Surya surface preference can override it"
            .into(),
    );
    macro_rules! apply {
            ...
            if let Some((key, value)) = first_color(&theme.colors, $keys, &mut report.warnings) {
                *$target = value;
                report.mappings.push(ImportMapping {
                    surya_role: $role.into(),
                    vscode_key: key.into(),
                    value: value.to_string(),
                });
            } else {
                report.fallbacks.push(format!(
                    "{} retained the Surya {} fallback",
                    $role,
    ...
    if let Some(candidate) = report.accent_candidates.first() {
        ...
        report
            .fallbacks
            .push("accent.* retained the Surya fallback; curate a native accent".into());
    }
    ...
    for key in candidate_keys {
        ...
        if minimum_contrast(candidate, backgrounds) >= minimum {
            ...
            if let Some(mapping) = report
                .mappings
                .iter_mut()
                .find(|mapping| mapping.surya_role == role)
            {
...
mod tests {
    ...
    fn hardens_ayu_style_low_contrast_workbench_roles_without_blocking_import() {
        ...
        assert!(imported.report.adjustments.iter().any(|adjustment| {
            adjustment.surya_role == "text" && adjustment.reason.contains("editor.foreground")
        }));
        assert!(
            imported
                .report
    ...
    fn hardens_and_reports_low_contrast_faint_text() {
        ...
        let adjustment = imported
            ...
            .adjustments
            .iter()
            .find(|adjustment| adjustment.surya_role == "textFaint")
        ...
        let mapping = imported
            ...
            .mappings
            .iter()
            .find(|mapping| mapping.surya_role == "textFaint")
            .unwrap();
    ...
    fn flattens_translucent_foundational_surfaces() {
        ...
        assert!(imported.report.adjustments.iter().any(|adjustment| {
            adjustment.surya_role == "background"
