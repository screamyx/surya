...
/// The app gate (surya's App.tsx phases). Pure.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GatePhase {
...
/// Session-row sub-line, "project · branch" (surya `chatLocation`): the repo
...
/// Per-kind chip label + one-line detail. Labels match surya's `describeTool`
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
    ...
    // Capitalize the first segment only (surya's style).
...
/// read the same on every surface. `surya-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `surya/<name>`
