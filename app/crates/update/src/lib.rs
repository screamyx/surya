//! surya-update — release checking and self-update, shared by the engine (the
//! background checker + `ApplyUpdate`), the CLI (`surya update`), and the UI
...
//! - **Managed** (`~/.surya/app/<ver>` + `current` symlink — the curl|sh
...
/// `surya-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("surya-{version}-{os}-{arch}.tar.gz")
}

/// `surya-<ver>-macos-<arch>-app.tar.gz` — the macOS app update payload.
pub fn mac_app_artifact(version: &str) -> String {
    let (_, arch) = platform_key();
    format!("surya-{version}-macos-{arch}-app.tar.gz")
}
...
fn http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent(concat!("surya/", env!("CARGO_PKG_VERSION")))
        .build()
...
pub enum InstallKind {
    /// `~/.surya/app/<ver>/surya` behind the `current` symlink
    /// (curl|sh installer / a previous `surya update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".surya").join("app");
...
pub async fn stage_headless(
    ...
    let result = async {
        ...
        if !unpacked.join("surya").is_file() {
            bail!("tarball {file} did not contain a surya binary");
        }
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("surya").exists() => {}
            Err(err) => {
...
/// Restart the installed engine service (the same units `surya daemon` and the
...
pub fn restart_service() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/sh.surya.app")],
        )
    } else {
        run("systemctl", &["--user", "restart", "surya.service"])
    }
}
...
/// Download + unpack the app tarball into `{data_dir}/updates/<ver>/Surya.app`
...
pub async fn stage_mac_app(
    ...
    let staged = dir.join("Surya.app");
    if staged.join("Contents/MacOS/surya").exists() {
        return Ok(staged);
    }
    ...
    if !staged.join("Contents/MacOS/surya").exists() {
        bail!("app tarball {file} did not contain Surya.app");
    }
    Ok(staged)
}
...
/// `SURYA_AUTO_UPDATE=1|true|yes` — headless daemons apply updates themselves.
fn auto_update_enabled() -> bool {
    std::env::var("SURYA_AUTO_UPDATE")
...
/// stream). Managed installs with `SURYA_AUTO_UPDATE` set stage + apply + service
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.surya/app/0.1.1/surya"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.surya/app")
            }
        );
        assert_eq!(
            detect_install_from(
                Path::new("/Applications/Surya.app/Contents/MacOS/surya"),
                Some(Path::new("/Users/u")),
            ),
            InstallKind::MacApp {
                bundle: PathBuf::from("/Applications/Surya.app")
            }
        );
        // A path merely containing `.app` without the bundle layout is not a bundle.
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/surya"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/surya"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("surya-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("surya-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"surya-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["surya-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("surya"), ver).unwrap();
        }
