...
use surya_doc::{MessagePart, MessageRole, SessionCommandPayload, SessionMessageEntry};
use surya_proto::{
...
/// Expanded-mode textarea vertical padding: `pt-4 pb-1` (surya composer.tsx
/// line 578) = 16 + 4.
...
/// 260)` (surya composer.tsx line 235). The 76px floor applies even when
...
/// children; composer/styles.tsx pickerChip) + `pb-2.5` (10) — surya
...
/// Staged-attachment strip metrics (surya attachment-ui.tsx AttachmentStrip:
...
const FILE_MENTION_SCHEME: &str = "surya-file:";
...
fn mention_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older surya — update it to search its files".into()
        }
...
fn slash_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older surya — update it to list commands".into()
        }
        RpcError::Transport(_) | RpcError::Closed => "The session's device is unreachable".into(),
...
pub struct Composer {
    ...
    /// `Window` — the `SURYA_ATTACH_PREVIEW` boot knob opens in `new`).
...
/// The whole notice decision for one batch, in one place.
///
...
impl Composer {
    ...
    fn answer_permission(
        ...
        cx.spawn(async move |this, cx| {
            let result = engine
                .client()
                .call(surya_rpc::methods::RESPOND_PERMISSION, params)
                .await;
    ...
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // a rig) — `SURYA_ATTACH=/path/a.png[,/path/b.png]`, and
        // `SURYA_ATTACH_PREVIEW=1` boots with the first one's lightbox open.
        if let Ok(spec) = std::env::var("SURYA_ATTACH") {
            let staged: Vec<StagedAttachment> = spec
                ...
                .filter_map(|path| {
                    match attachments::stage_file(std::path::Path::new(path.trim())) {
                        Ok(att) => Some(att),
                        Err(err) => {
                            tracing::warn!(%path, error = %err, "SURYA_ATTACH stage failed");
                            None
                        }
                    }
                })
    ...
    /// Capture-knob passthrough (`SURYA_OPEN_DIALOG=model`): open the
    ...
    /// Does NOT touch `self.failure`: it is shared with the paste and drop
    /// paths, and `add_paths` owns the notice for its whole batch through
    /// [`batch_notice`]. Clearing here as well would race that decision.
    ...
    fn send(&mut self, text: String, steer: bool, cx: &mut Context<Self>) {
        ...
        self.send_task = Some(cx.spawn(async move |this, cx| {
            let result: Result<(), String> = async {
                ...
                let mut run_worktree: Option<surya_proto::WorktreeSpec> = None;
                ...
                if is_new {
                    match &plan {
                        ...
                        crate::pickers::CheckoutPlan::NewWorktree { base } => {
                            ...
                            // actual surya/<name> branch post-creation. cwd
                            ...
                            if let Some(repo_path) = &space_path {
                                ...
                                run_worktree = Some(surya_proto::WorktreeSpec {
    ...
    /// The agent-asked-a-question panel (surya question-panel.tsx), rendered in
    ...
    fn render_wizard(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let options = question.options.iter().enumerate().map(|(ix, label)| {
            ...
            // (typed answers win — surya question-panel.tsx `isSel`).
            ...
            div()
                ...
                // surya question-panel.tsx option rows: `transition-colors`.
                .bg(if picked {
    ...
    fn render_send_button(
        ...
        let theme = Theme::of(cx);
        // Surya composer-actions.tsx: a size-7 filled circle — up-arrow to
...
impl Render for Composer {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Centered composer column (surya `mx-auto w-full max-w-3xl`).
        let container = div()
            .w_full()
            ...
            .when_some(failure, |el, message| {
                // surya composer.tsx `Notice` (matches the transcript
        ...
        let steer_queues = mode == SendButtonMode::Steer
            && self.pickers.read(cx).resolved_steering_mode(cx)
                == Some(surya_proto::SteeringMode::TurnBoundary);
        ...
        // need the full-width actions row (surya composer-actions.tsx
        // `mustExpand = isNew || …`).
        ...
        let attach = div()
            ...
            .rounded_full()
            .cursor_pointer()
            // surya composer-actions.tsx attach: `transition-colors`.
            .bg(motion::hover_blend(
                "composer-attach",
                gpui::transparent_black(),
        ...
        // The pill chrome (surya composer.tsx): `rounded-[26px] border
...
mod tests {
    ...
    fn file_mentions_serialize_to_strict_local_markdown() {
        ...
        assert_eq!(
            raw,
            "[a file#\\[x\\].rs](surya-file:src/a%20file%23%5Bx%5D.rs)"
        );
        let links = file_mention_links(&raw);
        ...
        assert_eq!(folder, "[components](surya-file:src/components/)");
    ...
    fn sent_mention_display_leaves_plain_prompts_untouched() {
        ...
        assert_eq!(
            sent_mention_display("what is a surya-file: link?"),
            None,
            "scheme substring without a valid mention link"
        );
        assert_eq!(
            sent_mention_display("[a.rs](surya-file:../a.rs)"),
            None,
    ...
    #[test]
    fn auto_grow_math() {
        // The source heights (surya composer.tsx line 235 clamp, composer-
        ...
        // Caps at a 260px textarea box (surya max-h-[260px] / the JS clamp).
        assert_eq!(
