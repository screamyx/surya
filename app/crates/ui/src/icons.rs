...
//!
//! The set mirrors the original surya's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in surya's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of surya's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
    ...
    // Mirrored variant (surya window-controls.tsx `-scale-x-100`): the LEFT
    ...
    // Hand-drawn surya glyphs (terminal-panel.tsx / composer-actions.tsx /
...
/// The Claude mark's brand orange (`#D97757`) — surya keeps it even on the
/// monochrome surface.
