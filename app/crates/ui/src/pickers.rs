...
use surya_engine::registry::HarnessDescriptor;
use surya_proto::{
...
/// Dev/testing knob: `SURYA_SLOW_CATALOG_MS=<ms>` delays every harness and
...
/// (headless-rig captures; same family as `SURYA_OPEN_PICKER`).
fn slow_catalog_delay() -> Option<std::time::Duration> {
    std::env::var("SURYA_SLOW_CATALOG_MS")
        .ok()
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (surya mints a `surya/<name>`
...
/// or foreign level — surya use-run-config.ts's derived-model discipline).
...
pub fn browser_rows(listing: &FolderListing) -> Vec<&surya_proto::FolderEntry> {
...
pub struct Pickers {
    ...
    config: DraftConfig,
    /// Sticky last-used picks (surya `surya.composer.defaults:v1`): seeds the
    ...
    focus: FocusHandle,
    /// `SURYA_OPEN_PICKER` boot: keep claiming focus until it sticks, so
...
impl Pickers {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // Dev/testing knob: `SURYA_OPEN_PICKER=model|traits|repo|branch` boots
        // with that popover open — synthetic input can't reach the app on
        // headless compositors, so captures need a data-side path.
        let boot_open = match std::env::var("SURYA_OPEN_PICKER").ok().as_deref() {
    ...
    fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
        ...
        // model (it stays available under `SURYA_HARNESS=mock`).
        self.harnesses
            .ready()
    ...
    pub fn resolved_steering_mode(&self, cx: &App) -> Option<surya_proto::SteeringMode> {
        let harness = self.effective_harness(cx)?;
        self.harnesses
            .ready()
    ...
    /// Capture knob (`SURYA_OPEN_DIALOG=model`): open the combined
    ...
    fn device_rows(&self, cx: &App) -> Vec<surya_proto::Device> {
        let state = self.state.read(cx);
        let local = state.local_device_id.clone();
        let mut devices: Vec<surya_proto::Device> = state.devices.clone();
    ...
    fn filtered_device_rows(&self, cx: &App) -> Vec<surya_proto::Device> {
        let query = self.search.read(cx).text().to_string();
    ...
    fn trigger_chip(
        ...
        // Ghost pill (surya composer/styles.tsx `pill`): `h-8 rounded-lg px-2.5
        ...
        div()
            ...
            // surya composer/styles.tsx `pill`: `transition-colors` — the wash
    ...
    fn popover_frame(&self, width: f32, content: AnyElement, cx: &mut Context<Self>) -> AnyElement {
        ...
        popover::popover_card(&theme)
            .w(px(width))
            // surya caps its tallest picker at min(640px, 75vh).
            .max_h(px(640.0))
    ...
    /// picker's rail + list panes bleed to the card edge (surya
    ...
    /// The combined harness + model switcher (surya harness-model-picker.tsx):
...
pub(crate) fn normalize_model_rows(harness: HarnessId, models: Vec<Model>) -> Vec<Model> {
    ...
    let catalog = match harness {
        HarnessId::ClaudeCode => surya_harness::claude::catalog::static_models(),
        _ => Vec::new(),
    };
        ...
        .filter_map(|mut model| {
            ...
            if let Some(base) = strip_1m(&model.id.clone()) {
                ...
                if !model.options.iter().any(|o| o.id == "contextWindow") {
                    model.options.push(surya_proto::ModelOption {
                        id: "contextWindow".into(),
                        label: "Context Window".into(),
                        choices: vec![
                            surya_proto::ModelOptionChoice {
                                id: "200k".into(),
                                label: "200K".into(),
                            },
                            surya_proto::ModelOptionChoice {
...
/// `SURYA_HARNESS=mock` (the e2e/dev rig) opts the mock harness into the UI;
/// production launches never set it, so the mock never surfaces there.
fn mock_harness_enabled() -> bool {
    surya_proto::env_compat::var("HARNESS")
        .ok()
        .as_deref()
        .map(str::trim)
        == Some("mock")
...
/// `SURYA_HARNESS=mock` shows it; otherwise it only remains when it's
...
fn offered_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
    visible_harnesses_impl(list, allow_mock)
        ...
        .filter(|d| {
            d.installed
                && (surya_engine::registry::descriptor_enabled(d)
...
impl Render for Pickers {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx).clone();
        // A SURYA_OPEN_PICKER popover never went through `toggle`, so claim
        ...
        // Chip shows the model's display name alone (surya `modelText`); the
        ...
        // attach + send after this element (surya composer-actions.tsx
        // arrangement).
        let left = div()
            .flex()
            .flex_row()
...
mod tests {
    use super::*;
    use surya_proto::{FolderEntry, Model, ModelOption, ModelOptionChoice};
    ...
    fn descriptor(id: HarnessId, name: &str) -> HarnessDescriptor {
        HarnessDescriptor {
            ...
            enabled: Some(true),
            reasoning_levels: Vec::new(),
            steering_mode: surya_proto::SteeringMode::StepBoundary,
    ...
    fn completion_prefix_lengths() {
        ...
        assert_eq!(completion_prefix_len("surya", "surya"), Some(5));
        assert_eq!(completion_prefix_len("surya", ""), Some(0));
        assert_eq!(completion_prefix_len("surya", "dev"), None);
    ...
    fn browser_navigation_reducer() {
        ...
        assert_eq!(browser_rows(&listing)[1].name, "surya");
    }
    ...
    fn mock_harness_hidden_unless_alone() {
        ...
        // …and opted back in by SURYA_HARNESS=mock (the e2e rig).
