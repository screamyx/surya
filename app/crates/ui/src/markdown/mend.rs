...
//! `remend`, ported to surya's incremental parser).
//!
...
//! handlers don't apply here — surya renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "surya:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](surya:pending-link)");
        mends("[docs](", "[docs](surya:pending-link)");
        mends("see [do", "see [do](surya:pending-link)");
        mends("![alt](https://x/i.p", "![alt](surya:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](surya:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](surya:pending-link)");
        mends("**a [b", "**a [b](surya:pending-link)**");
    }
