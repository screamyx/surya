...
use surya_syntax::{HighlightKind, HighlightSpan, HighlightedDocument};
...
/// Gap between markdown blocks inside one message (surya mdBlockGap).
pub const MD_BLOCK_GAP: f32 = 12.0;
/// Body text size / line height (surya: 14px / 22px).
...
// Table metrics — a port of mugen-markdown 0.6.2's `TableBlock` under surya's
...
/// Uniform cell padding in px (surya `table.cellPadding`).
pub const TABLE_CELL_PADDING: f32 = 12.0;
/// Hairline between rows in px (surya `table.gap`).
pub const TABLE_DIVIDER: f32 = 1.0;
/// Header row font weight (surya `table.headerWeight` = 700).
...
/// Minimum rendered column width in px, padding included (surya
...
/// Hairline tone (surya md theme `table.borderColor`: rgba(255,255,255,0.1)).
...
/// Tight monochrome heading scale (surya: h2 ≈ 16px semibold; headings step
...
/// A GFM table — a port of mugen-markdown's `TableBlock` under surya's md
...
/// surya's `table.headerWeight`; strong runs never drop below semibold).
fn flatten_runs_weighted(runs: &[InlineRun], theme: &Theme, base_weight: FontWeight) -> FlatText {
    ...
    for run in runs {
        ...
        // Links stay monochrome — foreground with an underline (surya's md
...
fn render_code_block(
    ...
    let mut block = div()
        ...
        .rounded(px(10.0))
        // Faint white wash over the near-black panel ≈ #101010 (surya's code
