...
use crate::motion::{self, SURYA_PULSE};
...
/// The floating-menu surface (surya `.glass-surface` + `menuSurface`):
...
/// One menu row (surya `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (surya's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (surya
...
/// Hairline divider between menu sections (surya `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (surya
...
/// A bordered trailing menu section (surya picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (surya dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (surya dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (surya:
