...
/// the scheme and the host, make a missing port explicit (the engine's
/// default, the same one the Add dialog assumes), drop a bare trailing slash.
/// A path or query is kept as typed, and `wss://` stays distinct from `ws://`
/// because it is a different endpoint.
pub fn canonical_url(url: &str) -> String {
    ...
    let default_port = super::DEFAULT_PORT.to_string();
