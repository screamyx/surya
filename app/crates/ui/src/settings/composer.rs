...
//! (surya parity: localStorage `surya.composer.defaults:v1`, defaults.ts).
//!
...
use surya_proto::{HarnessId, ReasoningLevel};

const FILE_NAME: &str = "composer-defaults.json";

/// Remembered model per harness — id plus display label, mirroring surya's
...
pub struct ComposerDefaults {
    ...
    /// Last reasoning level picked (global, like surya's `reasoning` key).
...
impl ComposerDefaults {
    ...
    /// Remember a pick (surya `saveDefaults({ harness, modelByHarness })`).
