...
pub struct ShortcutsPage {
    ...
    /// conflicts never persist; they're refused at record time, as in surya.
...
impl ShortcutsPage {
    ...
    fn on_key_down(&mut self, event: &KeyDownEvent, cx: &mut Context<Self>) {
        ...
        match record_key(
            ...
            RecordOutcome::Set(combo) => {
                ...
                // (surya settings.shortcuts.tsx: "… is already assigned to …").
    ...
    fn render_row(
        ...
        // surya settings.shortcuts.tsx row: min-h-[72px] px-5 gap-5.
        div()
            .min_h(px(72.0))
            .px(px(20.0))
            .flex()
...
/// One-line purpose copy per shortcut (surya lib/shortcuts.ts
...
impl Render for ShortcutsPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // the message names the specific clash (surya settings.shortcuts.tsx).
...
mod tests {
    ...
    fn conflicting_records_are_refused() {
        // surya parity: a combo bound elsewhere is refused at record time (the
