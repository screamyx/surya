...
use surya_proto::{ChangeRequestSummary, Chat, CheckoutChangeRequestStatus, Space};
...
impl ChangeRequestBadgeModel {
    pub fn from_summary(summary: &ChangeRequestSummary) -> Self {
        use surya_proto::ChangeRequestState;
...
mod tests {
    ...
    fn with_source(mut chat: Chat, branch: &str) -> Chat {
        chat.source_context = Some(surya_proto::ConversationSourceContext {
            checkout_id: "checkout".into(),
            repo_root: "/repo".into(),
            cwd: "/repo".into(),
            branch: branch.into(),
