//! Animation kit — the surya motion catalog as reusable helpers over gpui
...
//! - `surya-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// surya's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; surya never does).
...
/// Surya loader pulse period: 2.4s.
pub const SURYA_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (surya also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Surya-pulse floor opacity.
// The loader constants and math live in `surya_proto::motion` (pure phase
...
// the pointer enters. The original surya puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`SURYA_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `SURYA_MOTION_SCALE=10` slows the
...
pub fn speed_scale() -> f32 {
    ...
    *SCALE.get_or_init(|| {
        std::env::var("SURYA_MOTION_SCALE")
            .ok()
...
mod tests {
    ...
    #[test]
    fn catalog_timings_match_surya() {
        ...
        assert_eq!(SURYA_PULSE.duration_ms, 2400);
