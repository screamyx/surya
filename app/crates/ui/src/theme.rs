...
//! lightness steps; the same scale surya's Tailwind theme used) into gpui [`Hsla`].
...
use surya_syntax::HighlightKind;
use surya_theme::{
...
pub enum AccentColor {
    /// The exact upstream Surya indigo.
    #[default]
...
impl AccentColor {
    ...
    fn tokens(self, appearance: Appearance) -> AccentTokens {
        ...
        let (primary, strong) = match (self, appearance) {
            (Self::Surya, Appearance::Dark) => {
                (oklch(0.673, 0.182, 276.935), oklch(0.585, 0.233, 277.117))
            }
            (Self::Surya, Appearance::Light) => {
...
fn model_appearance(appearance: surya_theme::Appearance) -> Appearance {
    match appearance {
        surya_theme::Appearance::Dark => Appearance::Dark,
        surya_theme::Appearance::Light => Appearance::Light,
    }
}
...
impl Theme {
    ...
    /// Main-panel header height (surya `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (surya `h-6`) — the
    ...
    /// fixed Surya grey would erase that palette's identity.
    ...
    /// Increase tint coverage only as far as needed for Surya's shared text
    ...
    /// theme, so forcing frost does not reintroduce Surya's neutral hover.
    ...
    pub fn dark_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            terminal: TerminalColors::surya(Appearance::Dark),
    ...
    pub fn light_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            terminal: TerminalColors::surya(Appearance::Light),
            font_sans: "Geist".into(),
            font_sans_fixed: "Geist".into(),
            font_mono: "Geist Mono".into(),
            font_sans_fallback: system_sans().into(),
    ...
    pub(crate) fn from_variant(
        ...
        let accent_color = match accent_selection {
            AccentSelection::ThemeDefault => AccentColor::Surya,
...
mod tests {
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, surya's app background.
    ...
    #[test]
    fn surya_accent_is_the_exact_upstream_default() {
        let dark = Theme::dark();
        let light = Theme::light();
        assert_eq!(dark.accent_color, AccentColor::Surya);
    ...
    fn theme_recommendation_and_surface_override_resolve_independently() {
        ...
        let surya = Theme::dark();
        assert_ne!(catppuccin.surface, surya.surface);
        ...
        let opaque_surya = Theme::for_selection(
            Appearance::Dark,
            "zeron-dark",
            AccentSelection::ThemeDefault,
            SurfacePreference::Opaque,
        );
        assert_eq!(opaque_surya.surface_treatment, SurfaceTreatment::Opaque);
        assert_eq!(opaque_surya.glass(), opaque_surya.surface);
    }
