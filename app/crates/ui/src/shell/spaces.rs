...
fn compare_sidebar_chats(
    sort: SidebarSort,
    left: &surya_proto::Chat,
    right: &surya_proto::Chat,
) -> std::cmp::Ordering {
...
impl Shell {
    ...
    pub(super) fn sidebar_visible_order(&self, cx: &Context<Self>) -> Vec<String> {
        let filter = self.settings.space_filter.clone();
        let state = self.state.read(cx);
        let mut chats: Vec<surya_proto::Chat> = state
        ...
        let mut groups: Vec<(Option<(String, String)>, Vec<surya_proto::Chat>)> = Vec::new();
        for chat in chats {
    ...
    pub(super) fn render_archived_section(
        ...
        let now = Utc::now();
        let filter = self.settings.space_filter.clone();
        let mut rows: Vec<surya_proto::Chat> = {
            let state = self.state.read(cx);
            state
                .chats
                .iter()
    ...
    fn add_space_filtered(&self, cx: &App) -> Vec<surya_proto::FolderEntry> {
    ...
    pub(super) fn render_add_space_overlay(
        ...
        // ── breadcrumbs ("MacBook Pro / Projects / surya"): the quiet mono
...
mod tests {
    ...
    fn chat(id: &str) -> surya_proto::Chat {
        surya_proto::Chat {
            id: id.into(),
