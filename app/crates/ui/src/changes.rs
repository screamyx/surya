...
use surya_proto::{Chat, CheckoutDiff, GitHistoryCommit};
...
pub struct DiffHighlights {
    pub old: Option<Arc<surya_syntax::HighlightedDocument>>,
    pub new: Option<Arc<surya_syntax::HighlightedDocument>>,
}
...
impl DiffHighlights {
    ...
    pub fn spans(&self, line: &DiffLine) -> &[surya_syntax::HighlightSpan] {
...
fn excerpt_side(
    ...
    for hunk in &file.hunks {
        ...
        let source = visible
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let document = surya_syntax::highlight(surya_syntax::HighlightRequest {
            source: &source,
            path: Some(path),
            fence_tag: None,
        })
    ...
    Some(Arc::new(surya_syntax::HighlightedDocument {
        language,
        lines,
    }))
}
...
fn excerpt_highlights(file: &FileDiff, language: Lang) -> Option<DiffHighlights> {
    if !surya_syntax::supports_language(language) {
        return None;
    }
...
fn sources_match_patch(file: &FileDiff, response: &surya_proto::CheckoutFileDiffText) -> bool {
    let old = response
        .old_text
        .as_deref()
...
fn full_highlights(
    file: &FileDiff,
    language: Lang,
    response: &surya_proto::CheckoutFileDiffText,
    ...
    if old.is_none() && new.is_none() && surya_syntax::supports_language(language) {
        return None;
    }
    Some(DiffHighlights { old, new })
}
...
impl Changes {
    ...
    fn request_highlight(
        ...
        if !surya_syntax::supports_language(lang) {
        ...
        let fetch_task = match (active, engine) {
            (Some(diff), Some(engine)) => Some(cx.spawn(async move |this, cx| {
                let request = surya_proto::GetCheckoutFileDiffTextRequest {
                ...
                let response = engine
                    ...
                    .ok()
                    .and_then(|value| {
                        serde_json::from_value::<surya_proto::CheckoutFileDiffText>(value).ok()
                    });
    ...
    fn render_file_header(
        ...
        // Chevron (surya checkout-diff-sidebar): chevron-right closed,
...
fn diff_line_row(
    line: &DiffLine,
    spans: &[surya_syntax::HighlightSpan],
    theme: &Theme,
    gutter_px: f32,
...
/// column, and paint-only syntax runs (surya checkout-diff-sidebar).
        ...
        let scoped_notice: Option<(SharedString, bool)> = (!no_chat
            ...
            .map(|message| {
                if message.contains("no turn recorded") {
                    ...
                    (
                        SharedString::from(
                            "This chat's device is running an older Surya — update it to view branch and turn diffs",
                        ),
                        false,
                    )
                } else {
...
";
    ...
    fn phases() {
        ...
        summarized.files.push(surya_proto::DiffFileSummary {
            path: "x".into(),
            old_path: None,
    ...
    fn full_diff_highlights_map_old_new_and_context_by_source_line() {
        ...
        assert!(
            highlights
                .spans(&deleted)
                .iter()
                .any(|span| span.kind == surya_syntax::HighlightKind::Function)
        );
        assert!(
            highlights
                .spans(&added)
    ...
    fn split_line_runs_use_affected_old_and_new_documents() {
        ...
        for (path, source, required) in [
            (
                ...
                surya_syntax::HighlightKind::Tag,
            ),
            (
                "src/Greeter.kt",
                "fun greet(name: String) = println(name)",
                surya_syntax::HighlightKind::Function,
            ),
            (
                "Dockerfile",
                "RUN echo \"hello\"",
    ...
    fn excerpt_parses_old_and_new_hunks_as_separate_documents() {
        ...
        assert!(
            highlights
                .spans(deleted)
                .iter()
                .any(|span| span.kind == surya_syntax::HighlightKind::Comment)
        );
        assert!(
            highlights
                .spans(added)
    ...
    fn mismatched_full_sources_are_rejected_atomically() {
        ...
        let response = surya_proto::CheckoutFileDiffText {
