...
use surya_doc::{REGISTRY_DOC_ID, RegistryDoc};
use surya_proto::{Chat, Device, Session, SessionStatus};
use surya_sync::registry::mock_server::MockRegistryServer;
use surya_sync::{DocsStore, RegistryClient, RegistryEvent};
...
impl surya_sync::registry::RegistryTransport for FlakyAckTransport {
    fn fetch(
        &self,
        _since: u64,
...
async fn unreadable_http_ack_retries_instead_of_stranding_the_batch() {
    ...
    let client = RegistryClient::connect_via_transport(
        Arc::new(surya_sync::StaticUrl("ws://127.0.0.1:1/ws".into())),
        doc.clone(),
        "dev-a",
        surya_sync::RegistryTuning::default(),
