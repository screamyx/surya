...
//! handler works through the relay untouched (the port of surya's `device-room-host.ts`).
...
/// so the superseded process backs off and retries, mirroring surya's DeviceRoomHost).
...
async fn host_session(
    ...
    let ws = surya_sync::dial::connect_ws(url)
        .await
...
pub struct LinkCacheConfig {
    ...
    /// redialed at full cadence; callers fail fast in between (surya peers.ts behavior).
...
impl LinkCacheConfig {
    pub fn new(edge_url: impl Into<String>, token: Arc<dyn TokenSource>) -> Self {
        ...
        // this cache: one blip must cost seconds, not minutes. The old surya
...
/// Lazily-dialed, cached peer links keyed by device id — the Rust twin of surya's
