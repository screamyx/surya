...
A ground-up native rewrite of [surya](../surya) — a multi-device controller for coding agents
...
- Feature parity with surya **except token-usage display** (poor fit for CRDTs; excluded).
...
- **Engine = backend** (was `@surya/backend`): runs agents, owns auth, terminals, repos/worktrees,
...
- **Edge (TypeScript, ported from surya `apps/edge`)**: Worker + ChatRoom DO (per chat, the
...
### Headed / headless
Single binary `surya`:
- `surya` — headed. If a local engine daemon is already listening on the IPC port, connect to it;
...
- `surya headless` — engine only. A clean installation immediately serves its local profile over localhost IPC; when a saved account selects the synced profile at startup and a bearer is available, it also hosts its DeviceRoom for remote control. A VPS can run this while a laptop's UI drives it.
...
`surya login` and `surya logout` operate on `session.json` while the engine is stopped. Login selects `Synced` for the next start; logout selects `Local` for the next start. The UI may update live authentication status, but the active `WorkspaceScope` still changes only after restart.
...
3. **Mirror layer** (`surya-doc` crate) — Rust equivalent of loro-mirror: typed structs for the
   schema, **incremental** application of `doc.subscribe` diffs into cached state (no full
   re-hydration per change — this is also what fixes surya's known O(transcript) re-projection
...
This is surya's proven design, kept verbatim.
...
surya/
  ...
  crates/
    proto/        surya-proto    # wire types: AgentEvent, ToolCall, RunRequest, Model,
    ...
    doc/          surya-doc      # session-doc + workspace-registry schemas, mirror layer,
                                 # parts fold, continuations, command ledger, sidecars
    sync/         surya-sync     # loro room client (join/VV backfill/fragments/backoff),
                                 # ephemeral presence, DocsStore (SQLite snapshots +
                                 # processed-command ledger)
    harness/      surya-harness  # Harness trait + claude-code (stream-json subprocess),
                                 # codex (app-server JSON-RPC), mock; steering mailbox,
                                 # requestInput, models/reasoning/options catalogs
    engine/       surya-engine   # sessions engine (pub/sub, run journal, recovery, stall
    ...
    rpc/          surya-rpc      # UiRpc/ControlRpc: typed req/resp/stream over WS (tokio-
                                 # tungstenite) + in-memory transport; device-room virtual
                                 # sockets ({s,k,to,from} frames)
    theme/        surya-theme    # source-neutral theme schema + built-in/custom registry,
                                 # validation, provenance, and local VS Code compiler
    ui/           surya-ui       # gpui app: shell, sidebar, conversation, composer,
                                 # terminal view, diff pane, settings, animation kit
  apps/
    surya/                       # the binary (headed default, `headless` subcommand)
  edge/                          # TypeScript Worker + DOs (ported from surya/apps/edge,
...
- **Markdown** (`surya-ui::markdown`): `pulldown-cmark` parsing on `background_spawn` with
...
- **Animation kit** (`surya-ui::motion`): small helpers over gpui `Animation` reproducing the
  surya catalog — `fade-in` (0.5s, cubic-bezier(0.16,1,0.3,1), translateY 4→0), `splash-out`,
  `surya-pulse` staggered cell wave (boot splash + loaders), `gradient-spin-pulse` matrix
...
Direct ports of surya behaviors (spec: feature-inventory §3):
...
- **Doc host**: per-chat handle (join room, VV backfill, write user entries + stream assistant
  ...
  snapshot store.
- **Harness** (research pending — `docs/research/harness.md`): trait mirroring surya's
...
- **Repos/diffs**: git2 or `git` subprocess (subprocess — matches surya, avoids libgit2 edge
  cases); worktrees under `~/.surya/worktrees`; fs watchers (`notify`) + 2min repair; diff
...
Port `surya/apps/edge` nearly verbatim (it is already Loro-native and smoke-tested: session room
...
- **Changed**: Postgres entity sync/server → workspace registry + edge; Electron/React/mugen → gpui with
  ported techniques; Node harness SDKs → subprocess protocols; WebRTC → device-room relay (surya
...
- ✅ **M1 Doc + sync core** — `surya-doc` mirror over loro 1.13; room client syncs with the edge
...
- ✅ **M2 Engine core** — Claude harness end-to-end headless: `surya headless` + dev auth runs a
...
3. Cursor harness (surya has it; CLI surface for Rust TBD) — parity item, scheduled after Codex.
