//! `surya update` — check for and apply a newer release, natively (the same
...
use anyhow::bail;
use surya_update::{InstallKind, current_version, version_newer};
...
pub async fn update(edge_url: &str, check_only: bool) -> anyhow::Result<()> {
    ...
    if !version_newer(&manifest.version, current) {
        println!(
            "surya {current} is up to date (latest: {}).",
            manifest.version
        );
        return Ok(());
    }
    println!("surya {current} → {} available", manifest.version);
    if check_only {
        std::process::exit(1);
    }

    match surya_update::detect_install() {
        InstallKind::Managed { app_root } => {
            println!(
                "downloading {}…",
                surya_update::headless_artifact(&manifest.version)
            );
            surya_update::stage_headless(edge_url, &manifest, &app_root).await?;
            surya_update::apply_headless(&app_root, &manifest.version)?;
            println!(
                "installed {} (current → {})",
                app_root.join(&manifest.version).display(),
                manifest.version
            );
            match surya_update::restart_service() {
        ...
        InstallKind::MacApp { bundle } => {
            println!(
                "downloading {}…",
                surya_update::mac_app_artifact(&manifest.version)
            );
            let data_dir = surya_proto::env_compat::var_os("DATA_DIR")
                .map(std::path::PathBuf::from)
                .unwrap_or_else(super::dirs_data_dir);
            let staged = surya_update::stage_mac_app(edge_url, &manifest, &data_dir).await?;
            surya_update::apply_mac_app(&staged, &bundle)?;
            println!("updated {} — relaunch Surya to finish.", bundle.display());
            Ok(())
        }
        InstallKind::Unmanaged => {
            bail!(
                "this binary is not update-managed (source build or hand-copied).\n\
                 ...
                 macOS: download the new Surya.app dmg, or rebuild from source."
            )
        }
    }
}
