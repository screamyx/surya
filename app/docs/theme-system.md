...
Surya themes are complete, source-neutral `ThemeVariant` values owned by the
`surya-theme` crate. Runtime components consume only Surya semantic roles. VS
...
- Every variant records a recommended `SurfaceTreatment`: Surya recommends
...
instead of fixed Surya greys. Its window tint becomes denser when necessary to
...
still allowing the independent Surya surface preference to force frost.

After source mapping, a deterministic hardening pass checks Surya's shared
...
related source role when the preferred one cannot carry the Surya role, and
...
The `surya-theme-import` development tool exposes the same single-file adapter
for built-in curation:

Example:

```bash
cargo run -p surya-theme --bin surya-theme-import -- \
  --input /path/to/theme.json \
  --output /tmp/theme.surya.json \
...
source. Surya does not fetch or execute VSIX packages at runtime; custom
