...
- Run `cargo fmt --check`, the relevant `surya-ui` tests/checks, and a local app build.
...
- `cargo check -p surya-ui` passes with existing workspace warnings.
- `cargo test -p surya-ui --lib settings::appearance` passes (4 tests).
