...
actions for both Surya and the conversation's actual harness.
...
```text
ConversationLinks
  surya: required internal deeplink
...
The Surya URI must include an opaque workspace/profile locator in addition to
the durable chat id so local, synced, and development profiles cannot collide.
Proposed shape:

```text
surya://open/chat/<percent-encoded-chat-id>?workspace=<opaque-locator>
```

“Copy Surya deeplink” is complete only when the URI round-trips into the correct
...
1. Add the optional source-context wire/doc model in `surya-proto` and
   `surya-doc`; keep old rows readable.
...
2. Add a workspace-scoped Surya URI builder/parser and inbound route intent.
3. Register and handle `surya://` in macOS packaging first; add Linux desktop
...
```bash
cargo fmt --all -- --check
cargo test -p surya-proto
cargo test -p surya-doc
cargo test -p surya-engine change_request
cargo test -p surya-ui change_request
cargo test -p surya-ui shell
```

Broaden to `cargo test -p surya-engine`, `cargo test -p surya-ui`, and finally
