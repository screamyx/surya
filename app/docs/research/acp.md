...
- Add an **ACP harness** (`crates/harness/src/acp/`) speaking Agent Client Protocol
  ...
  NOT the official SDK crates: surya keeps its own child-lifecycle hardening
...
- First registered agent: **Grok Build** (`grok agent stdio`), xAI's native ACP
  ...
  OAuth or `XAI_API_KEY`; surya passes env through. `GROK_EXECUTABLE` overrides
...
- **claude/codex converted to ACP too** (2026-08-08, wing's call: "keep things
  ...
  adapter-owned; surya-specific settings ride config options where advertised
...
- **Hermes + Pi registered** (2026-08-08): `AcpHarness::hermes()` runs Nous
  ...
  entry); thinking ladder minimal→max maps onto surya's levels via the
  generic `thought_level` preference ladder ("off" has no surya tier).
- **Devin registered** (2026-08-15): `AcpHarness::devin()` runs Cognition's
  ...
  `cognition.ai/subagentSupport`; Surya correlates the child agent id back to
  ...
  `cognition.ai/subagentControl` extension stays disabled until Surya has a
...
- **Session config options**: ACP has no per-prompt model field; the run's
  ...
  `low`/`medium`/`high`; other surya levels degrade down a preference ladder
  (`config_option_sets`).
...
- Replaced by `adapter_install`: pinned packages install ONCE into
  `~/.surya/adapters/<pkg>/<version>` (`$SURYA_ADAPTERS_DIR` overrides) with
  a surya-owned npm cache beside them, atomically (tmp dir + bin-entry
  verification + `.surya-install-ok` marker + rename), then every launch is
